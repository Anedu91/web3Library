...
const initContractGlobalObject = async () => {
  if (window.CONTRACT_ADDRESS === "YOUR CONTRACT ADDRESS HERE") {
    alert(
      "You forgot to insert your NFT contract address in your Webflow Embed code. Insert your contract address, publish the website and try again. If you don't have it, contact https://buildship.xyz"
    );
    return;
  }
  // Default to Ethereum
...
export const fetchABI = async (address, chainID) => {
  ...
  const abi = await fetch(
    `https://metadata.buildship.xyz/api/info/${address}?network_id=${chainID}`
  )
    .then((r) => r.json())
    .then((r) => r.abi)
    .catch((e) => null);
...
const fetchCachedABI = async (address) => {
  ...
  try {
    ...
    alert(
      "Wrong format for ABI cache. Should be a URL to .json file. Fix or remove ABI cache URL to resolve"
    );
...
export const isEthereumContract = () =>
  [1, 4].includes(window.CONTRACT.nft.allowedNetworks[0]);
