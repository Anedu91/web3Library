...
const findMethodByName = (methodName) =>
  Object.keys(NFTContract.methods).find(
...
const getCustomMintTx = (numberOfTokens) => {
  if (window.DEFAULTS?.contractMethods?.mint) {
    console.log(
      "Using custom mint method name: ",
      window.DEFAULTS?.contractMethods?.mint
    );
    if (NFTContract.methods[window.DEFAULTS?.contractMethods?.mint]) {
      return NFTContract.methods[window.DEFAULTS?.contractMethods?.mint](
        numberOfTokens
      );
    } else {
      alert(
        "Custom mint method name isn't present in the ABI, using default parser"
      );
      console.log(
        "Custom mint method name isn't present in the ABI, using default parser"
      );
...
const getMintTx = ({ numberOfTokens, wallet }) => {
  const customMintTx = getCustomMintTx(numberOfTokens);
  if (customMintTx) return customMintTx;
  ...
  if (!name) {
    alert(
      "Buildship widget doesn't know how to mint from your contract. Contact https://buildship.xyz in Discord to resolve this."
    );
    return undefined;
  }
  return NFTContract.methods[findMethodByName(name)](numberOfTokens, wallet);
};
...
const getDefaultMintPrice = () => {
  ...
  if (defaultPrice) {
    const priceNumber =
      typeof defaultPrice === "string" ? Number(defaultPrice) : defaultPrice;
    if (isNaN(priceNumber)) {
      alert(
        "Wrong publicMintPrice format, should be a number in ETH (or native token)"
      );
      return undefined;
    }
    console.warn(
      "Using DEFAULTS.publicMint.price as price not found in the smart-contract"
    );
...
export const getMintPrice = async () => {
  const matches = Object.keys(NFTContract.methods).filter(
    (key) =>
      !key.includes("()") &&
...
export const getMaxSupply = async () => {
  ...
  if (NFTContract.methods.totalSupply)
    return await NFTContract.methods.totalSupply().call();
  alert(
    "Widget doesn't know how to fetch maxSupply from your contract. Contact https://buildship.xyz to resolve this."
  );
  return undefined;
};
...
export const mint = async (nTokens) => {
  ...
  if (mintPrice === undefined) return { tx: undefined };

  const txParams = {
    from: wallet,
    value: formatValue(Number(mintPrice) * numberOfTokens),
  };
  const estimatedGas = await getMintTx({ numberOfTokens, wallet })
    ...
    .catch((e) => {
      ...
      alert(
        `Error ${message}. Please try refreshing page, check your MetaMask connection or contact us to resolve`
      );
      console.log(e);
    });
  ...
  const maxFeePerGas = [1, 4].includes(chainID)
    ? formatValue(maxGasPrice)
    : undefined;
  const maxPriorityFeePerGas = [1, 4].includes(chainID) ? 2e9 : undefined;

  const tx = getMintTx({ numberOfTokens, wallet }).send({
    ...txParams,
